...
static __init int pchar_init(void) {
    ...
    // initialize sync for each device
    for(i=0; i<devcnt; i++) {
        //mutex_init(&devices[i].lock);
        spin_lock_init(&devices[i].lock);
        printk(KERN_INFO "%s: device %d sync is initizalized.\n", THIS_MODULE->name, i);
    }
    return 0;
...
static __exit void pchar_exit(void) {
    ...
    // destroy sync object for each device
    for(i=devcnt-1; i>=0; i--)
        ; //mutex_destroy(&devices[i].lock);
    printk(KERN_INFO "%s: destroyed sync objs for devices.\n", THIS_MODULE->name);
...
static ssize_t pchar_read(struct file *pfile, char * __user ubuf, size_t ubuf_size, loff_t *fpos) {
    ...
    //mutex_lock(&pdev->lock); //ret = mutex_lock_interruptible(&pdev->lock);
    spin_lock(&pdev->lock);
