...
static __init int pchar_init(void) {
    ...
    // initialize waiting queues of each device
    for(i=0; i<devcnt; i++) {
...
static ssize_t pchar_read(struct file *pfile, char * __user ubuf, size_t ubuf_size, loff_t *fpos) {
    ...
    // block the current process, if kfifo is empty
    ret = wait_event_interruptible(pdev->rd_wq, !kfifo_is_empty(&pdev->buf));
    if(ret != 0) {
        printk(KERN_INFO "%s: pchar_read() wakeup due to signal.\n", THIS_MODULE->name);
        return -EINTR; // user space write call will fail with error EINTR.
        //return -ERESTARTSYS; // user space write call will retried -- will block again.
    }
    ...
    // wakeup writer process if blocked
    if(nbytes > 0)
...
static ssize_t pchar_write(struct file *pfile, const char * __user ubuf, size_t ubuf_size, loff_t *fpos) {
    ...
    // block the current process, if kfifo is full
    ...
    // wakeup reader process if blocked
    if(nbytes > 0)
