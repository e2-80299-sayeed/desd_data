...
static __init int bbb_gpio_init(void) {
	...
	if(irq > 0) {
		...
		if(ret < 0)
			...
			printk(KERN_INFO "%s: SWITCH gpio pin irq is registered and tasklet initialized.\n", THIS_MODULE->name);
		}
	}
	return 0;
}
...
static irqreturn_t switch_isr(int irq, void *param) {
	printk(KERN_INFO "%s: switch_isr() called and tasklet scheduled.\n", THIS_MODULE->name);
	// toggle led -- to be done in bottom half (tasklet)
...
static void switch_tasklet_handler(unsigned long param) {
	printk(KERN_INFO "%s: switch_tasklet_handler() called -- LED toggled.\n", THIS_MODULE->name);
	led_state = ! led_state;
